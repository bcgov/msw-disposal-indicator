  ...
  mutate(Total_Disposed_Tonnes = case_when (
    ...
    TRUE ~ Total_Disposed_Tonnes),
         Disposal_Rate_kg = case_when (
           Year == 2024 ~ round((Total_Disposed_Tonnes) / Population * 1000), 
...
waste each B.C. resident sends to a landfill or other disposal site (e.g., incinerators) in a
given year.
...
-   Waste reduction efforts are more and more important as B.C. population continues to
    increase, landfills approach their capacity for receiving new waste, and the
    imperative to reduce greenhouse gas emissions from waste disposal grows.
...
-   In `r max_year`, British Columbians disposed of an
    ...
    waste per person---the lowest disposal rate recorded, improving the previous lowest of `r min_rate` kg/person in `r min_rate_year`.
...
#scale.x <- scale_x_continuous(breaks = c(seq(min(BC_data$Year), max(BC_data$Year)-3, by = 5), 2024))
#scale.x <- scale_x_continuous(breaks = sort(c(seq(max(BC_data$Year), min(BC_data$Year)+8, by = -5), 1994, 1990)))
scale.x <- scale_x_continuous(breaks = c(seq(min(BC_data$Year), max(BC_data$Year)-5, by = 4), 2021, 2024))
...
-   Regional district population estimates are sourced from [BC
    Stats](https://www2.gov.bc.ca/gov/content?id=36D1A7A4BEE248598281824C13CB65B6). As of 2024, population adjustments (e.g., for tourism) are no longer used.
...
-   The current version of the disposal rates for some regional districts may 
    differ from previous versions due to data improvements or correction by regional districts.
    The ministry keeps track of all the changes for future references. 
-   In `r max_year`, the ministry updated its methodology to minimize assumptions and 
    reduce the need for adjustments to disposal rates typically requested by regional districts. 
    This approach supports the publication of more consistent baseline data with fewer revisions, assumptions and data modifications. 
    Some significant changes between 2023 and 2024 data are due to this methodology change 
    (e.g., Fraser Valley Reginal District and Squamish-Lillooet Regional District).

Regional districts that wish to update the waste disposal data reported
above should contact [the Environmental Policy and Initiatives Branch](mailto:CircularCommunities@gov.bc.ca). Input from regional districts will
...
-   [Solid Waste Management Planning in B.C.                                                                          ](https://www2.gov.bc.ca/gov/content/environment/waste-management/garbage/municipal-waste-management-plans)

-   [British Columbia\'s *Environmental Management
    Act*](http://www.bclaws.ca/civix/document/id/complete/statreg/03053_00)

-   [British Columbia Ministry of Environment and Parks 2026/27 – 2028/29 Service Plan (February 2026) (PDF)](https://www.bcbudget.gov.bc.ca/2026/sp/pdf/ministry/env.pdf)
...
-   [Municipal Solid Waste Disposal, 1990-`r max_year`](https://catalogue.data.gov.bc.ca/dataset/d21ed158-0ac7-4afd-a03b-ce22df0096bc) (Licence: [Open Government Licence - British Columbia](http://www2.gov.bc.ca/gov/content?id=A519A56BC2BF44E4A008B33FCF527F61))
